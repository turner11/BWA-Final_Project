...
cudaError_t GetDevSeqsA(bwa_seq_t *seqs,bwa_seq_t **dev_seqsPtr, int n_seqs)
{
	cudaError_t cudaStatus1;
	return cudaStatus1;
	bwa_seq_t* dev_seqs = *dev_seqsPtr;
	// Allocate GPU buffers for sequences
	cudaError_t cudaStatus = cudaMalloc((void**)&dev_seqs,n_seqs * sizeof(bwa_seq_t));
	if (cudaStatus != cudaSuccess) {
		fprintf(stderr, "cudaMalloc failed!");
		goto Error;
	}

	// Copy input structs from host memory to GPU seqs.
	cudaStatus = cudaMemcpy(dev_seqs, seqs, n_seqs * sizeof(bwa_seq_t), cudaMemcpyHostToDevice);
	if (cudaStatus != cudaSuccess) {
	...
	for (i= 0; i < n_seqs; i++)
	{
		bwa_seq_t *currSeq = seqs + i;
		bwa_seq_t *currDevSeq = seqs + i;

		/*Name*/
		int nameLength = strlen(currSeq->name)+1;
		cudaError_t cudaStatus = cudaMalloc((void**)&(currDevSeq->name),nameLength);
		if (cudaStatus != cudaSuccess) {
			fprintf(stderr, "cudaMalloc failed!");
			goto Error;
		}
		...
		// Copy input structs from host memory to GPU seqs.
		cudaStatus = cudaMemcpy(currDevSeq->name, currSeq->name, nameLength, cudaMemcpyHostToDevice);
		if (cudaStatus != cudaSuccess) {
			fprintf(stderr, "cudaMemcpy failed!");
			goto Error;
		}
...
__global__ void bwa_cal_sa_reg_gapKernel(bwt_t *const bwt, int n_seqs, bwa_seq_t *seqs, const gap_opt_t *opt)
{
...
cudaError_t bwa_cal_sa_reg_gapWithCuda(int tid, bwt_t *const bwt, int n_seqs, bwa_seq_t *seqs, const gap_opt_t *opt)
{
...
// 	}
// 
// 	// Allocate GPU buffers for argument structs
// 	cudaStatus = cudaMalloc((void**)&dev_opt,sizeof(gap_opt_t));
// 	if (cudaStatus != cudaSuccess) {
// 		fprintf(stderr, "cudaMalloc failed!");
// 		goto Error;
// 	}
...
// 
// 
// 	// Copy input structs from host memory to GPU buffers.
// 	cudaStatus = cudaMemcpy(dev_opt, opt, sizeof(gap_opt_t), cudaMemcpyHostToDevice);
// 	if (cudaStatus != cudaSuccess) {
// 		fprintf(stderr, "cudaMemcpy failed!");
// 		goto Error;
// 	}
// 	cudaStatus = GetDevSeqsA(seqs,&dev_seqs);
// 	
// 	
// 
// 	/************************************************************************/
